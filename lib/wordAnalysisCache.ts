...
export function cacheKeyFor(wordText: string, root: string): string {
  return `${wordText.trim()}::${root.trim()}`;
}
...
function readCache(): WordAnalysisCache {
  ...
  try {
    ...
    return JSON.parse(fs.readFileSync(cachePath(), 'utf-8')) as WordAnalysisCache;
  } catch {
    return {};
  }
}
